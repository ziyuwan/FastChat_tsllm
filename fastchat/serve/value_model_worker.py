...
def tsllm_load_model(
    ...
    model = load_critic_model(
        ...
        device=device,
        value_model_type_name=value_model_type_name,
    )
    tokenizer = AutoTokenizer.from_pretrained(critic_model_path)
    return model, tokenizer


class ValueModelWorker(BaseModelWorker):
    def __init__(
        self,
        controller_addr: str,
        worker_addr: str,
        ...
        **kwargs,
        ...
        #     debug=debug,
        # )
        self.model, self.tokenizer = tsllm_load_model(
            critic_model_path=model_path, device=device
        )

        self.device = device
        if self.tokenizer.pad_token == None:
            self.tokenizer.pad_token = self.tokenizer.eos_token
        self.context_len = get_context_length(self.model.base_model.config)
    ...
    def value_inference_gate(self, params):
        ...
        if isinstance(input_str, list):
            indices2pick = torch.LongTensor(
                [len(self.tokenizer.encode(txt)) - 1 for txt in input_str]
            )
        else:
            indices2pick = torch.LongTensor([len(self.tokenizer.encode(input_str)) - 1])

        inputs = self.tokenizer(input_str, return_tensors="pt", padding=True).to(
            self.model.device
        )
        if "token_type_ids" in inputs:
            inputs.pop("token_type_ids")
        try:
            value = self.model(**inputs).value.cpu()
            value = (
                value.gather(1, indices2pick.unsqueeze_(1)).squeeze_(1).float().numpy()
            )
            ret = {"value": value.tolist()}
            # return json.loads(value.tolist())
            gc.collect()
            torch.cuda.empty_cache()
        except torch.cuda.OutOfMemoryError as e:
            ret = {
